...
class _PayrollPayrunScreenState extends State<PayrollPayrunScreen> {
      ...
      builder: (dialogContext) {
        return BackdropFilter(
          ...
          child: AlertDialog(
            ...
            title: const Text('Add Payee (Employee/Supplier)', style: TextStyle(color: Colors.white)),
            content: Form(
              key: formKey,
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    TextFormField(
                      style: const TextStyle(color: Colors.white),
                      decoration: const InputDecoration(labelText: 'BSB (e.g. 062-900)'),
                      validator: (v) => (v == null || v.isEmpty) ? 'Required' : null,
                      onSaved: (v) => bsb = v ?? '',
                    ),
                    TextFormField(
                    ...
                    TextFormField(
                      style: const TextStyle(color: Colors.white),
                      decoration: const InputDecoration(labelText: 'Net Amount (AUD)'),
            ...
            actions: [
              ...
              ElevatedButton(
                style: ElevatedButton.styleFrom(backgroundColor: IZiiColors.primary),
                onPressed: () {
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          bottom: TabBar(
            ...
            tabs: [
              const Tab(text: 'STP Payrun'),
              const Tab(text: 'History'),
              const Tab(text: 'ABA Bank Payments'),
            ],
          ),
        ),
  ...
  Widget _buildDatePickerRow(String label, DateTime currentVal, ValueChanged<DateTime> onChanged) {
    return InkWell(
      onTap: () async {
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: IZiiColors.darkSurfaceHighlight.withOpacity(0.2)),
              ),
              child: Column(
                children: [
                  _buildDatePickerRow('Pay Period Start', _periodStart, (d) => setState(() => _periodStart = d)),
                  const Divider(color: Colors.white12),
                  _buildDatePickerRow('Pay Period End', _periodEnd, (d) => setState(() => _periodEnd = d)),
                  const Divider(color: Colors.white12),
                  _buildDatePickerRow('Payment Date', _paymentDate, (d) => setState(() => _paymentDate = d)),
                ],
              ),
            ),
            const SizedBox(height: 20),
            Text('STP payroll values (STP Phase 2 schema mapping)', style: TextStyle(color: Colors.white.withOpacity(0.5), fontSize: 13)),
              ...
              child: Column(
                children: [
                  ...
                  TextFormField(
                    ...
                    onChanged: (v) {
                      ...
                      setState(() {
                        ...
                        _paygTax = double.parse((val * 0.22).toStringAsFixed(2));
                        // 11% Superannuation (Mandatory SG Rate FY2024-25 / FY2025-26)
                        _superAmount = double.parse((val * 0.11).toStringAsFixed(2));
                      });
                    },
                  ),
                  const SizedBox(height: 12),
                  ...
                  TextFormField(
                    ...
                    initialValue: _superAmount > 0 ? _superAmount.toString() : '',
                    style: const TextStyle(color: Colors.white),
                    decoration: const InputDecoration(
                    ...
                    keyboardType: TextInputType.number,
                    validator: (v) => (v == null || v.isEmpty) ? 'Required' : null,
                    onSaved: (v) => _superAmount = double.tryParse(v ?? '') ?? 0.0,
                  ),
                ],
              ),
            ),
            ...
            ElevatedButton(
              ...
              child: const Row(
                ...
                children: [
                  ...
                  SizedBox(width: 8),
                  Text('Submit STP to ATO', style: TextStyle(fontWeight: FontWeight.bold)),
                ],
              ),
            ),
          ],
  ...
  Widget _buildStpHistoryTab(List<Map<String, dynamic>> events) {
    if (events.isEmpty) {
      return const Center(child: Text('No historical payruns logged', style: TextStyle(color: Colors.white38)));
    }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, idx) {
        final ev = events[idx];
        return Card(
          color: IZiiColors.darkSurface.withOpacity(0.5),
          ...
          child: ExpansionTile(
            ...
            trailing: Container(
              ...
              decoration: BoxDecoration(
                color: IZiiColors.success.withOpacity(0.15),
                borderRadius: BorderRadius.circular(8),
              ),
              child: const Text(
                'ATO Lodged',
                style: TextStyle(color: IZiiColors.success, fontWeight: FontWeight.bold, fontSize: 11),
              ),
            ),
            children: [
              Padding(
                ...
                child: Column(
                  children: [
                    ...
                    _buildHistoryRow('PAYG Withholding (W2)', ev['total_tax_withheld']),
                    _buildHistoryRow('Superannuation', ev['total_super']),
                    const Divider(color: Colors.white12),
                    _buildHistoryRowStr('STP Lodgment Receipt', ev['stp_receipt_number'] ?? 'N/A'),
                  ],
                ),
              ),
            ],
          ),
        ).animate().fadeIn(delay: (idx * 50).ms, duration: 300.ms).slideY(begin: 0.05, end: 0);
      },
    );
  }
  ...
  Widget _buildHistoryRowStr(String label, String val) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Text(label, style: const TextStyle(color: Colors.white54, fontSize: 13)),
          Text(val, style: const TextStyle(color: IZiiColors.accent, fontWeight: FontWeight.bold, fontSize: 12)),
        ],
      ),
    );
  }
              ...
              child: Column(
                children: [
                    ...
                    children: [
                      Expanded(
                        child: TextFormField(
                          ...
                          style: const TextStyle(color: Colors.white),
                          decoration: const InputDecoration(labelText: 'Bank Code (e.g. CBA, NAB)'),
                          validator: (v) => (v == null || v.isEmpty) ? 'Required' : null,
                      ...
                      Expanded(
                        child: TextFormField(
                          ...
                          style: const TextStyle(color: Colors.white),
                          decoration: const InputDecoration(labelText: 'APCA number (6 digits)'),
                          keyboardType: TextInputType.number,
                  ...
                  TextFormField(
                    ...
                    style: const TextStyle(color: Colors.white),
                    decoration: const InputDecoration(labelText: 'Supplying User Name'),
                    validator: (v) => (v == null || v.isEmpty) ? 'Required' : null,
                    ...
                    children: [
                      Expanded(
                        child: TextFormField(
                          ...
                          style: const TextStyle(color: Colors.white),
                          decoration: const InputDecoration(labelText: 'Paying BSB'),
                          validator: (v) => (v == null || v.isEmpty) ? 'Required' : null,
                      ...
                      Expanded(
                        child: TextFormField(
                          ...
                          style: const TextStyle(color: Colors.white),
                          decoration: const InputDecoration(labelText: 'Paying Account Number'),
                          validator: (v) => (v == null || v.isEmpty) ? 'Required' : null,
              ...
              itemBuilder: (context, idx) {
                ...
                return Dismissible(
                  ...
                  child: Container(
                    ...
                    child: ListTile(
                      title: Text(p['account_title'], style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
                      subtitle: Text('BSB: ${p['bsb']} • Acc: ${p['account_number']} • Ref: ${p['lodgement_reference']}', style: TextStyle(color: Colors.white.withOpacity(0.4), fontSize: 11)),
                      trailing: Text('\$${p['amount_dollars'].toStringAsFixed(2)}', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                    ),
                  ),
                );
              },
            ...
            ElevatedButton(
              ...
              child: const Row(
                ...
                children: [
                  ...
                  SizedBox(width: 8),
                  Text('Generate ABA Bank Payment File', style: TextStyle(fontWeight: FontWeight.bold)),
                ],
              ),
            ),
          ],
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      IconButton(
                        icon: const Icon(Icons.copy_all_rounded, color: IZiiColors.primary),
                        onPressed: () {
                          Clipboard.setData(ClipboardData(text: content));
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('ABA content copied to clipboard')),
                          );
                        },
                      ),
                    ],
