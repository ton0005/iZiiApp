...
class BleTransportService {
  ...
  static const int maxFragmentSize = 480; // Leaving room for header within 512 bytes MTU
  static const int compressionThreshold = 100; // Compress if payload > 100 bytes
  ...
  List<BleFragment> fragmentPayload(List<int> payload, BleMessageType originalType) {
    ...
    for (var i = 0; i < payload.length; i += maxFragmentSize) {
      final end = (i + maxFragmentSize < payload.length) ? i + maxFragmentSize : payload.length;
  ...
  ({List<int> payload, BleMessageType type})? addFragment(BleFragment fragment) {
    final fragmentIdHex = fragment.fragmentId.map((b) => b.toRadixString(16).padLeft(2, '0')).join();
    ...
    if (buffer.length == expected) {
      ...
      for (var i = 0; i < expected; i++) {
        if (buffer.containsKey(i)) {
          ...
          print('[BleTransport] Error: Missing fragment index $i in group $fragmentIdHex');
          return null;
        }
      }
