...
class BleDeviceDiscoveryService {
  static final BleDeviceDiscoveryService _instance = BleDeviceDiscoveryService._internal();
  ...
  final _nearbyPeersController = StreamController<List<LocalBlePeer>>.broadcast();

  Stream<List<LocalBlePeer>> get nearbyPeersStream => _nearbyPeersController.stream;
  ...
  Future<void> startAdvertising() async {
    ...
    try {
      ...
      _isAdvertising = true;
      print('[BleDiscovery] Started BLE Advertising. Service UUID: $serviceUuid');
    } catch (e) {
  ...
  Future<void> _handleDiscoveredDevice(ScanResult result) async {
    ...
    final String deviceId = 'izii-d-ble-${result.device.remoteId.str.replaceAll(':', '').toLowerCase()}';
    ...
    print('[BleDiscovery] Discovered peer: $deviceName ($deviceId) RSSI: ${result.rssi}');
  }
  ...
  Future<bool> connectAndAuthenticate(String deviceAddress) async {
    ...
    try {
      ...
      if (p2pChar == null) {
        throw Exception('iZii BLE P2P Characteristic not found on target device.');
      }

      print('[BleDiscovery] Starting Noise Handshake...');
      final initiator = 'ble-${deviceAddress.replaceAll(':', '').toLowerCase()}';
      ...
      // Wait for Message 2
      final msg2 = await responseCompleter.future.timeout(const Duration(seconds: 10));
      subscription.cancel();

      // Step 3: Process Message 2 and write Message 3
      final msg3 = await _handshakeService.processHandshakeMessage(initiator, msg2);
      if (msg3 == null) {
        throw Exception('Noise Handshake failed at Message 2 processing.');
      }

      await p2pChar.write(msg3);
      print('[BleDiscovery] Noise Handshake established successfully with $deviceAddress.');
      ...
      if (state != null) {
        ...
        final String peerDeviceId = 'izii-d-ble-${deviceAddress.replaceAll(':', '').toLowerCase()}';
        await (_db.update(_db.localBlePeers)..where((t) => t.deviceId.equals(peerDeviceId)))
            .write(LocalBlePeersCompanion(
              publicKey: Value(base64Encode(msg2.sublist(32, 64))), // Extracted static X25519 public key
            ));
      }
