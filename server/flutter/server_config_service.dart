...
///   await config.init();                 // gọi 1 lần lúc app khởi động
///   final current = config.currentServer; // null nếu chưa từng chọn server nào
///   await config.setCurrentServer(info);  // lưu lựa chọn mới
///
/// Các BLoC/Repository khác trong app nên đọc `config.currentServer?.baseUrl`
/// để biết gọi API vào đâu, thay vì hard-code IP.
  ...
  Future<void> init() async {
    ...
    if (raw != null) {
      try {
        ...
        // Dữ liệu lưu bị hỏng/lỗi format cũ — coi như chưa chọn server.
        _currentServer = null;
      }
    }
    _initialized = true;
